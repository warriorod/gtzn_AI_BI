...
from constants.code_enum import DataTypeEnum, DiFyCodeEnum, IntentEnum, SysCodeEnum
        ...
        try:
            ...
            async with aiohttp.ClientSession(read_bufsize=1024 * 16) as session:
                async with session.post(
                    ...
                    headers=headers,
                    json=body_params,
                    timeout=aiohttp.ClientTimeout(total=60 * 36),  # 等待36分钟超时，与 TASK_TIMEOUT(30min) + 缓冲 对齐
                ) as response:
