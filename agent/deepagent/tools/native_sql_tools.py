...
from .schema_retriever import bm25_retrieve_tables, format_schema_text
from .tool_call_manager import (
    get_tool_call_manager,
)

_BM25_TOP_K = int(os.getenv("DEEP_AGENT_BM25_TOP_K", "20"))
...
def set_native_datasource_info(
    datasource_id: int, datasource_type: str, datasource_config: str, session_id: Optional[str] = None
):
    """设置原生数据源信息"""
    ...
    try:
        with db_pool.get_session() as session:
            ...
            tables = (
                ...
                .filter(DatasourceTable.ds_id == datasource_id, DatasourceTable.checked == True)
                .all()
            )
...
def sql_db_query(query: str) -> str:
    ...
    try:
        ...
        # 执行查询（添加超时控制）
        result_data = DatasourceConnectionUtil.execute_query(datasource_type, config, query)
        ...
        if result_rows:
            ...
            for col in columns:
                col_widths[col] = min(
                    max(len(str(col)), max(len(str(row.get(col, ""))[:50]) for row in result_rows)), 50
                )
            ...
            for row in result_rows:
                row_str = " | ".join(str(row.get(col, ""))[:50].ljust(col_widths[col]) for col in columns)
                result_str += row_str + "\n"
...
def _get_table_relationships_from_datasource(table_names: Optional[list] = None) -> list:
    ...
    try:
        with db_pool.get_session() as session:
            ...
            # 解析节点和边
            table_nodes = [r for r in relations if r.get("shape") in ("er-rect", "rect")]
            ...
            all_tables = session.query(DatasourceTable).filter(DatasourceTable.ds_id == datasource_id).all()
                ...
                if node:
                    ...
                    for p in ports:
                        if str(p.get("id")) == str(port_id):
                            return p.get("attrs", {}).get("portNameLabel", {}).get("text", "").strip()
                # 2) 兜底：从 DatasourceField.id 读取
                try:
                    if port_id and str(port_id).isdigit():
                        field = session.query(DatasourceField).filter(DatasourceField.id == int(port_id)).first()
...
@tool
def sql_db_smart_search(user_query: str) -> str:
    """ALWAYS call this tool FIRST before sql_db_list_tables or sql_db_schema.

    Intelligently retrieves the most relevant database tables for a given query
    and returns their full schema (table comments, column names, types, comments)
    in a single call — replacing the two-step sql_db_list_tables + sql_db_schema flow.

    Uses BM25 keyword matching to rank tables by relevance to the user query.
    When the total number of tables is small, returns all tables directly.

    Args:
        user_query: The user's original question or requirement (Chinese or English).

    Returns:
        Full schema of the Top-K most relevant tables, plus their names for use
        with sql_db_table_relationship.
    """
    logger.info("[smart_search] 工具被调用, user_query=%r", user_query)
    datasource_id, _, _ = _get_datasource_info()
    logger.info("[smart_search] datasource_id=%s", datasource_id)
    if not datasource_id:
        return "错误: 未设置数据源信息"
    ...
    allowed, reason = _check_tool_call("sql_db_smart_search")
    if not allowed:
        logger.warning("[smart_search] tool_call_manager 拒绝: %s", reason)
        return reason

    try:
        all_table_info = _get_table_info_from_metadata()
        total_count = len(all_table_info)
        logger.info("[smart_search] 元数据加载完成，共 %d 张表", total_count)

        if not all_table_info:
            _record_tool_call("sql_db_smart_search", False)
            return "错误: 数据库中没有可用的表"

        # 表数量 <= TOP_K 时跳过检索，直接返回全量
        if total_count <= _BM25_TOP_K:
            logger.info("[smart_search] 表数(%d) <= TOP_K(%d)，跳过BM25直接返回全量", total_count, _BM25_TOP_K)
            selected_names = list(all_table_info.keys())
        else:
            logger.info("[smart_search] 启动 BM25 检索，TOP_K=%d", _BM25_TOP_K)
            selected_names = bm25_retrieve_tables(all_table_info, user_query, top_k=_BM25_TOP_K)
            if not selected_names:
                logger.warning("[smart_search] BM25 结果为空，降级返回全量前 %d 张表", _BM25_TOP_K)
                selected_names = list(all_table_info.keys())[:_BM25_TOP_K]

        schema_text = format_schema_text(selected_names, all_table_info)
        _record_tool_call("sql_db_smart_search", True)

        header = (
            f"✅ 智能检索完成：从 {total_count} 张表中筛选出 {len(selected_names)} 张相关表。\n"
            f"相关表：{', '.join(selected_names)}\n"
            f"（如需其他表的 schema，可额外调用 sql_db_schema）\n"
        )
        footer = "\n\n✅ schema 已获取完成。请直接基于以上信息编写 SQL，无需重复调用此工具。"
        return header + schema_text + footer

    except Exception as e:
        _record_tool_call("sql_db_smart_search", False)
        logger.error(f"sql_db_smart_search 失败: {e}", exc_info=True)
        return f"智能搜索失败: {str(e)[:100]}\n" "请降级使用 sql_db_list_tables + sql_db_schema 获取表结构。"
