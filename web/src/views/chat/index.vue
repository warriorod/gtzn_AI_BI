...
const handleCreateStylized = async (
  ...
  if ((currentQaType === 'DATABASE_QA' || currentQaType === 'REPORT_QA') && !selectedDatasource.value) {
...
const mobileSidebarMedia = typeof window !== 'undefined'
  ? window.matchMedia('(max-width: 768px)')
  : null
const collapsed = ref(mobileSidebarMedia?.matches ?? false)
const isMobileViewport = ref(mobileSidebarMedia?.matches ?? false)

function syncMobileSidebarState(event?: MediaQueryListEvent) {
  const isMobile = event?.matches ?? mobileSidebarMedia?.matches ?? false
...
onMounted(async () => {
  mobileSidebarMedia?.addEventListener('change', syncMobileSidebarState)
...
// 在组件卸载前移除事件监听
onBeforeUnmount(() => {
  mobileSidebarMedia?.removeEventListener('change', syncMobileSidebarState)
...
const handleSubmitFromDefaultPage = (payload: { text: string, mode: string, datasource_id?: number, selected_skills?: string[] }) => {
  ...
  activeModeValue.value = payload.mode === 'COMMON_QA' ? null : payload.mode
...
const handleDatasourceModeTrigger = (mode: string) => {
  if (document.activeElement instanceof HTMLElement) {
    document.activeElement.blur()
  }
  if (activeModeValue.value === mode) {
...
const handleHistoryClick = async (item: any) => {
  ...
  // 先设置对话类型
  onAqtiveChange(item.qa_type, item.chat_id)
  activeModeValue.value = item.qa_type === 'COMMON_QA' ? null : item.qa_type
                    ...
                    >
                      ...
                      >
                        <template #trigger>
                          <div
                            class="mode-icon-btn"
                            :class="{ active: activeModeValue === opt.value }"
                            :style="{
                              '--active-color': opt.color,
                              '--active-bg': `${opt.color}15`,
                            }"
                            @click.stop="handleDatasourceModeTrigger(opt.value)"
                          >
                            <div
                              :class="opt.icon"
                              class="text-14"
                      ...
                      >
                        <template #trigger>
                          <div
                            ...
                            @click.stop="toggleMode(opt.value)"
                          >
                            <div
                              :class="opt.icon"
                              class="text-14"
...
@media (max-width: 768px) {
  .qianwen-sidebar {
    ...
    z-index: 40;
    max-width: 82vw;
    box-shadow: 10px 0 30px rgb(15 23 42 / 18%);
  }

  .qianwen-sidebar.n-layout-sider--collapsed {
    box-shadow: none;
  }
...
@media (max-width: 768px) {
  ...
  .mode-icon-label,
  .mode-pill span,
  .mode-icon-btn .i-hugeicons\:arrow-down-01 {
    display: none;
  }

  .mode-icon-btn .text-14,
  .mode-pill .text-16,
  .action-icon {
